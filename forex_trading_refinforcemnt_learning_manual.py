...
class ForexTradingEnv(gym.Env):
    ...
    def __init__(self, df, output_dir, initial_balance=1000, window_size=12, lot_size=10000):
    ...
    def step(self, action):
        ...
        if action == 1:  # Buy
            ...
            elif self.position == -1:  # Close an existing Sell position and open a Buy
                profit = (self.entry_price - current_price) * self.lot_size
                self.net_worth += profit  # Update net worth based on profit
                self.position = 1  # Switch to a Buy position
                self.entry_price = current_price
            ...
            if self.position == 0:  # Open a new Sell position
                self.position = -1
                self.entry_price = current_price
            elif self.position == 1:  # Close an existing Buy position and open a Sell
                profit = (current_price - self.entry_price) * self.lot_size
                self.net_worth += profit  # Update net worth based on profit
                self.position = -1  # Switch to a Sell position
                self.entry_price = current_price

        elif action == 0:  # Hold
            reward += 0  # No action, no reward

        # Update net worth for open positions
        #if self.position == 1:  # Update for open long positions
           # unrealized_profit = (current_price - self.entry_price) * self.lot_size
           # self.net_worth = self.net_worth + unrealized_profit
       # elif self.position == -1:  # Update for open short positions
           # unrealized_profit = (self.entry_price - current_price) * self.lot_size
          #  self.net_worth = self.net_worth + unrealized_profit

        if self.net_worths:
            reward += self.net_worth - self.net_worths[-1]

        unrealized_high = 0
        unrealized_low = 0

        # Update net worth based on unrealized high and low
        if self.position == 1:  # Long position
            ...
            if unrealized_high < 100:
                reward += unrealized_high
            elif unrealized_low < 100:
                reward += unrealized_low
        ...
        #if self.net_worth <= self.initial_balance - 1000:
            # reward -= 100
            # done = True
...
class PPOAgent:
    ...
    def select_action(self, state):
        # Flatten the state and convert to a tensor
        ...
        action_probs = torch.softmax(action_logits, dim=-1).cpu().detach().numpy()
